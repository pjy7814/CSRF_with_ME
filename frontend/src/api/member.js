...
async function findById(memberId, success, fail) {
  ...
  await api.post(`/member/info`, memberId).then(success).catch(fail);
}
...
// async function getMemberInfo(memberId, success, fail) {
//   api.defaults.headers["access-token"] = sessionStorage.getItem("access-token");
//   await api.post(`/member/info`, memberId).then(success).catch(fail);
// }
...
async function updateMember(member, success, fail) {
  await api.post(`/member/update`, JSON.stringify(member)).then(success).catch(fail);
}

export { login, findById, tokenRegeneration, registMember, updateMember };
